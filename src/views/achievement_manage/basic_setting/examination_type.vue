<template>
    <section class="cm-container">
            ...
            <el-form :inline="true" size="mini" :model="param">
                <div class="fold fold-lt">
                    <div ref="formDetail" class="detail">
                        ...
                        <el-form-item>
                            <el-button size="mini" type="primary" @click="addExaminationType"  v-if="btnPerObj.add">新增</el-button>
                        </el-form-item>
                    </div>
                </div>
            </el-form>
        ...
        >
            <el-form :model="addForm" ref="addForm" class="boxStyle" :rules="ruless" >
        ...
        methods: {
           ...
           getBtnPermit() {
                ...
                this.$http.get('_auth:/privilege/getList/'+actionId+'/2').then(data=>{
                    data.data.map(item => {
                        if (item.code=="add_kaoshileixing") {
                            self.btnPerObj.add=true;
                        }
                        if (item.code=="edit_kaoshileixing") {
                            self.btnPerObj.edit=true;
                        }
                        if (item.code=="delete_kaoshileixing") {
                            self.btnPerObj.delete=true;
                        }
                        if (item.code=="query_kaoshileixing") {
...
//          this.getBtnPermit();//按钮权限
        ...
        created(){
            this.getExaminationList();//初始化请求数据
        },
        computed:{}
    }
</script>
