        ...
        <el-dialog
            ...
            <el-form :model="formDialog" ref="formDialog">
                <el-form-item v-for="(item,index) in editFormDetail" :key="index"
                    ...
                    <el-input v-model="formDialog[item.prop]" :placeholder="'请输入'+item.label.substring(2,4)"
                              :disabled="item.editDisabled && dialog.status!=='add'"></el-input>
                </el-form-item>
                
                <el-form-item label="层次" class="flex-form" prop="educationalLevelId" :rules="commonRules('请选择层次', formDialog.educationalLevelId, formDialog, 'educationalLevelId')">
                    <el-select v-model="formDialog.educationalLevelId"  placeholder="请选择层次" style="width:100%;">
...
<script>
    ...
    const TYPE='专业门类',REMARK='professional-category';
...
//          };
            return {
                ...
                pagePort:{
                    table:'_op:majorCategory/list',
                    change:'_op:majorCategory',
                ...
                tableLoading:false,
                editFormDetail:[
                    {label:'门类代码',prop:'code',editDisabled:true,rule:true},
                    {label:'门类名称',prop:'cnName',editDisabled:false,rule:true},
                    {label:'英文名称',prop:'enName',editDisabled:false,rule:true,}
                ],
                optionItem:[],
                  btnPerObj:{//按钮权限
                  query:false
           ...
           getBtnPermit() {
            ...
            commonRules(msg,val,formDialog,item){
            	let text_number = (rule, value, callback) =>{
	            	...
	            	}else if(msg.indexOf("英文")!= -1 && value.length>64){
	            		return callback(new Error("名称字数不能超过64个字"));
	            	}else{
	            		return callback();
	            	}
            	}
            ...
            commonRequire(method){
                ...
                if(method==='post'){
                    ...
                    // option.educationalLevelName && delete option.educationalLevelName;
            ...
            editItem(row){
                this.$http.get(this.pagePort.change+'/'+row.id)
                    .then(data=>{
                    	// data.data.educationalLevelId=data.data.educationalLevelId.toString()
