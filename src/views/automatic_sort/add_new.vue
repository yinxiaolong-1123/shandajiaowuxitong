  ...
  <section class="cm-container">
    ...
    <div class="cm-table">
      ...
      >
        ...
        <el-table-column prop="termName" label="星期四"></el-table-column>Z
      ...
      <div id="vueBox">
        <div class="square" v-for="(item,$index) in squareList" :key="$index">
          <ul class="flex">
            <li
              @drop="drop($index,'index',$event)"
              draggable="true"
...
export default {
  ...
  methods: {
    ...
    drop(index, attr, e) {
      ...
      if (
        Math.abs(e.clientX - that.clientX) >= 0 &&
        Math.abs(e.clientX - that.clientX) <= $(".square li").width() / 2
      ) {
        var str = that.squareList[that.moveIndex][attr];
        that.squareList[that.moveIndex][attr] =
          that.squareList[that.targetIndex][attr];
        that.squareList[that.targetIndex][attr] = str;
      }
    },
    allowDrop(event) {
      event.preventDefault();
